    ...
    return (<>
      ...
      <section className="flex items-start justify-center min-h-screen py-12 px-4">
        <div className="flex flex-col lg:flex-row bg-gray-800 rounded-lg shadow-lg overflow-hidden w-full max-w-6xl">
          ...
          {/* Left Side: User Info */}
          <ProfileCard avatar={Lock} // You can replace this with user's avatar if available
    ...
    />
          ...
          <main className="w-full lg:w-2/3 bg-stone-900 p-6">
            ...
            <div className="mb-6 flex justify-end">
              <a onClick={handleCreateRoomClick} // Trigger navigation on click
     className="text-lg text-blue-100 hover:underline cursor-pointer">
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {roomsCreated && roomsCreated.map((room) => (<RoomCard key={room.id} // Unique key for each room
         title={room.title} // Room title
         description={room.description} // Room description
         buttonText="View Details" buttonColor="bg-red-500"/>))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {rooms && rooms.map((room) => (<RoomCard key={room.id} // Unique key for each room
         title={room.title} // Room title
         description={room.description} // Room description
         buttonText="View Details" buttonColor="bg-green-500"/>))}
              </div>
            </div>
          </main>
