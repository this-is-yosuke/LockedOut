...
const CountdownTimer = ({ onTimeUp }) => {
    ...
    return (<div className="text-2xl font-semibold text-stone-100 mb-6">
      Time Remaining: {formatTime(timeLeft)}
    </div>);
};
// Modal Component for Time Up
const TimeUpModal = ({ onClose }) => (<div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex justify-center items-center z-50">
    <div className="bg-white p-8 rounded-lg text-center">
      ...
      <button onClick={onClose} className="py-2 px-6 bg-blue-500 text-white rounded-md hover:bg-blue-600">
        Go to Home
      </button>
    </div>
  </div>);
...
const EscapeRoom = () => {
    ...
    const fetchRiddles = async (roomID) => {
        try {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 p-2">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-4">
                {riddles && riddles.length > 0 ? (riddles.map((riddle) => (<div key={riddle.id} className="bg-stone-700 p-4 rounded-md">
                      <p className="text-lg text-stone-100">
                        {riddle.position}. {riddle.content}
                      </p>
                    </div>))) : (<div>No riddles available</div>)}
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <form onSubmit={handleSubmit} className="flex justify-between mt-8">
                {riddles.map((riddle, index) => (<input key={riddle.id} type="text" name={`riddle${index + 1}`} value={answers[`riddle${index + 1}`] || ''} onChange={handleInputChange} className="w-16 h-16 text-2xl text-center bg-stone-600 text-stone-100 rounded-md focus:outline-none focus:ring-2 focus:ring-stone-500" maxLength={1} placeholder={riddle.position.toString()}/>))}
              </form>

              <div className="text-center mt-6">
                <button type="submit" onClick={handleSubmit} className="py-2 px-6 bg-blue-500 text-white text-lg rounded-md hover:bg-red-600">
                  Unlock the Lock
                </button>
              </div>
            </div>
